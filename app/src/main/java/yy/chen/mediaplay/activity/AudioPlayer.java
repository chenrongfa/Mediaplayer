            ...
            public void handleMessage(Message msg) {
                  switch (msg.what) {
                        ...
                        case AUDIOSEEKTO:
                              ...
                              tvTime.setText(TimeUtil.TimeToString(currentionplayPosition) + "/" + TimeUtil.TimeToString(duration));
...
//        startService(intent);
     ...
     private File file;
       ...
       * @param name
       */
      private void resolveLyric( final String name) {
          //开线程去查找
                        ...
                        if(externalStorageState.equals(Environment.MEDIA_MOUNTED)){
                              if(name!=null){
                                    int i = name.lastIndexOf("/");
                                     singName=name.substring(i+1,name.lastIndexOf("."));//歌名
                                    Log.e(TAG, "resolveLyric:singName "+singName );
                                  String  name1= name.substring(0, i);
                                    String name2= name1.substring(0,name1.lastIndexOf("/"));//得到上一层目录;
                                    file=new File(name2);
                              }
                              File[] files = getFiles(file, singName);
                              Log.e(TAG, "run:files "+files.toString() );
                              //准备歌词
                              try {
                                    for (int num=0;num<files.length;num++){
                                          lyrics = LyricUtil.getLyricFromFile(files[num]);
                                          if(lyrics.size()>0){
                                                break;
                                          }
                                    }
      ...
      private static File[] getFiles(final File file, final String singName) {
            File[] files = file.listFiles(new FileFilter() {
                  @Override
                  public boolean accept(File pathname) {
                        if(pathname.isDirectory()){
                            getFiles(pathname, singName);

                        }else {
                              String name = pathname.getName();
                              int i = name.lastIndexOf("/");
                              if(i!=-1) {
                                    String lyric = name.substring(i + 1);//歌词
                                    //歌词前缀
                                    String preLyric = lyric.substring(0,lyric.lastIndexOf("."));
                                    //后缀
                                    String nextLyric=lyric.substring(lyric.lastIndexOf(".")+1);
                                    if(singName.equals(preLyric)){
                                          if(nextLyric.equals("txt")||nextLyric.equals("lrc")){
